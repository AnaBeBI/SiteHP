...
export async function POST(request: Request) {
  ...
  const { userId, inicio, fim } = await request.json();
  if (!userId || !inicio || !fim) {
    return NextResponse.json({ error: 'Usuário, início e fim são obrigatórios' }, { status: 400 });
  }
  ...
  const { data, error } = await adminClient
    .from('pontos')
    .insert({ user_id: userId, inicio, fim, criado_por: user.id })
